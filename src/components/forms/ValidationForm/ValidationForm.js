...
const ValidationForm = ({ smaData, trainingSize, model, setValidateData }) => {
  const onSubmit = () => {
    ...
    const val_train_y = makePredictions(val_train_x, model.model, model.normalize)
    
    const val_unseen_x = inputs.slice(Math.floor(trainingSize / 100 * inputs.length), inputs.length);
    const val_unseen_y = makePredictions(val_unseen_x, model.model, model.normalize)
    
    const timesstamps = smaData.map(data => data.timestamp)
